...
const ApiClient = {
  ...
  async callGemini(params, settings) {
    ...
    if (isDescribe) {
      userInstruction = `
[REVERSE PROMPT TASK]: Meticulously and objectively analyze the visual elements of the uploaded image. Reverse-engineer it into a world-class, production-ready prompt for ChatGPT Images 2.5 that faithfully reproduces this exact image.
Directly identify and extract the image's OWN authentic artistic medium, visual style, actual aspect ratio and camera framing, lighting setup, color palette, camera optics/focal length, and spatial composition as present in the image itself.
DO NOT apply any external preset styles, preset aspect ratios, or preset composition constraints.

ANTI-LAZINESS PROTOCOL (CRITICAL):
- DO NOT be brief, lazy, or output a 1-sentence summary.
- The "optimizedPrompt" MUST be an exhaustive, highly detailed paragraph of 100 to 180 words.
- Explicitly cover: (1) exact artistic medium/camera film stock, (2) subject anatomy, clothing texture and micro-details, (3) lens focal length (e.g. 85mm f/1.4) and depth of field, (4) directional key/rim lighting with Kelvin color temperature, (5) atmospheric environment depth, background layers, and color grading.
...
Please produce the structured JSON prompt following the Images 2.5 Brief format instructions.`;
  ...
  async callOpenAICompatible(params, settings) {
    ...
    const ratioNote = params.aspectRatio ? `\nTarget Aspect Ratio: "${params.aspectRatio}" (Weave matching composition and framing description).` : '';
    const avoidNote = (params.avoidTags && params.avoidTags.length > 0) ? `\nElements to Strictly Avoid: "${params.avoidTags.join(', ')}".` : '';
    ...
    if (isDescribe) {
      userContent.push({
        type: 'text',
        text: `[REVERSE PROMPT TASK]: Meticulously and objectively analyze the visual elements of the uploaded reference image. Reverse-engineer it into a world-class, production-ready prompt for ChatGPT Images 2.5 that faithfully reproduces this exact image.
Directly identify and extract the image's OWN authentic artistic medium, visual style, actual aspect ratio and camera framing, lighting setup, color palette, camera optics/focal length, and spatial composition as present in the image itself.
DO NOT apply any external preset styles, preset aspect ratios, or preset composition constraints.

ANTI-LAZINESS PROTOCOL (CRITICAL):
- DO NOT be brief, lazy, or output a 1-sentence summary.
- The "optimizedPrompt" MUST be an exhaustive, dense paragraph of 100 to 180 words.
- Explicitly cover: (1) exact artistic medium/camera film stock, (2) subject anatomy, clothing texture and micro-details, (3) lens focal length (e.g. 85mm f/1.4) and depth of field, (4) directional key/rim lighting with Kelvin color temperature, (5) atmospheric environment depth, background layers, and color grading.
