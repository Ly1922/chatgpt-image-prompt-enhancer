...
Please produce the structured JSON prompt following the Images 2.5 Brief format instructions.`;
    ...
    const requestBody = {
      system_instruction: {
        parts: [{ text: PromptEngine.SYSTEM_PROMPT }]
      },
      contents: [
        {
          role: 'user',
  ...
  async callOpenAICompatible(params, settings) {
    ...
    const ratioNote = params.aspectRatio ? `\nTarget Aspect Ratio: "${params.aspectRatio}" (Incorporate framing suitable for this ratio).` : '';
    const avoidNote = (params.avoidTags && params.avoidTags.length > 0) ? `\nNegative Constraints (Elements to avoid): "${params.avoidTags.join(', ')}".` : '';

    userContent.push({
      type: 'text',
      text: `User Rough Prompt: "${params.roughPrompt || '(Based entirely on reference images)'}"
Selected Generation Mode: "${params.mode || 'new'}"
Preferred Style Preset: "${params.style || 'photorealistic'}"${ratioNote}${avoidNote}${multiImageNote}
