...
const ApiClient = {
  ...
  async callGemini(params, settings) {
    ...
    if (isDescribe) {
      userInstruction = `
[REVERSE PROMPT TASK]: Meticulously and objectively analyze the visual elements of the uploaded image. Reverse-engineer it into a world-class production prompt for ChatGPT Images 2.5 that faithfully reproduces this exact image.
...
Please produce the structured JSON prompt following the Images 2.5 Brief format instructions.`;
  ...
  async callOpenAICompatible(params, settings) {
    ...
    if (isDescribe) {
      userContent.push({
        type: 'text',
        text: `[REVERSE PROMPT TASK]: Meticulously and objectively analyze the visual elements of the uploaded reference image. Reverse-engineer it into a world-class production prompt for ChatGPT Images 2.5 that faithfully reproduces this exact image.
Directly identify and extract the image's OWN authentic artistic medium, visual style, actual aspect ratio and camera framing, lighting setup, color palette, camera optics/focal length, and spatial composition as present in the image itself.
DO NOT apply any external preset styles, preset aspect ratios, or preset composition constraints.
